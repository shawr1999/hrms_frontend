...
export default function Payee_create() {
...
//   const [branchAddress, setBranchAddress] = useState('');
  ...
  const [PayeeId, setPayeeId] = useState('');
    ...
    useEffect(() => {
        fetch('https://shawr1999.pythonanywhere.com//api/payees/')
            ...
            .then(data => {
                ...
                const table = $(`#${tableName}`).DataTable({
                    data: data.map(Payee => [
                        ...
                        `<button class="edit-btn btn btn-primary" data-id="${Payee.id}">Edit</button>
                         <button class="delete-btn btn btn-danger" data-id="${Payee.id}">Delete</button>`
                    ]),
                    columns: [
                        { title: "Name" },
                        { title: "Number" },
    ...
    const handleEdit = (event) => {
        const PayeeId = $(event.currentTarget).data('id');
        navigate(`/edit-Payee/${PayeeId}`);
    };
...
const handleSubmit = (e) => {
    ...
    axios.post('https://shawr1999.pythonanywhere.com//api/payees-create/', formData)
        .then(response => {
            Swal.fire({
                title: "Good job!",
        ...
        setPayeeId(response.data.Leave_id);
      })
      .catch(error => {
  ...
  if (result.isConfirmed) {
    // 
    const PayeeId = $(event.currentTarget).data('id');
        fetch(`https://shawr1999.pythonanywhere.com//api/payees/${PayeeId}/`, { method: 'DELETE' })
            .then(response => {
                if (response.ok) {
                    setPayee(Payee.filter(Payee => Payee.id !== PayeeId));
                    Swal.fire({
      title: "Deleted!",
