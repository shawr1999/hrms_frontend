...
export default function Deposit_create() {
...
//   const [branchDescription, setBranchDescription] = useState('');
  ...
  const [DepositId, setDepositId] = useState('');
    ...
    useEffect(() => {
        fetch('https://shawr1999.pythonanywhere.com//api/deposits/')
            ...
            .then(data => {
                ...
                const table = $(`#${tableName}`).DataTable({
                    data: data.map(Deposit => [
                        ...
                        `<button class="edit-btn btn btn-primary" data-id="${Deposit.id}">Edit</button>
                         <button class="delete-btn btn btn-danger" data-id="${Deposit.id}">Delete</button>`
                    ]),
                    columns: [
                        { title: "Account" },
                        { title: "Amount" },
    ...
    const handleEdit = (event) => {
        const DepositId = $(event.currentTarget).data('id');
        navigate(`/edit-Deposit/${DepositId}`);
    };
...
const handleSubmit = (e) => {
    ...
    axios.post('https://shawr1999.pythonanywhere.com//api/deposits-create/', formData)
        .then(response => {
            Swal.fire({
                title: "Good job!",
        ...
        setDepositId(response.data.Leave_id);
      })
      .catch(error => {
  ...
  if (result.isConfirmed) {
    // 
    const DepositId = $(event.currentTarget).data('id');
        fetch(`https://shawr1999.pythonanywhere.com//api/deposits/${DepositId}/`, { method: 'DELETE' })
            .then(response => {
                if (response.ok) {
                    setDeposit(Deposit.filter(Deposit => Deposit.id !== DepositId));
                    Swal.fire({
      title: "Deleted!",
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <input type="text" className='form-control' value={Payer} onChange={(e) => setPayer(e.target.value)} required />
        </div>
          <div className="form-group">  
        <label>
         Deposit Name:
