...
//   const [branchAddress, setBranchAddress] = useState('');
  ...
  const [ContractId, setContractId] = useState('');
    ...
    useEffect(() => {
        fetch('https://shawr1999.pythonanywhere.com//api/contracts/')
            ...
            .then(data => {
                ...
                const table = $(`#${tableName}`).DataTable({
                    data: data.map(Contract => [
                        ...
                        `<button class="edit-btn btn btn-primary" data-id="${Contract.id}">Edit</button>
                         <button class="delete-btn btn btn-danger" data-id="${Contract.id}">Delete</button>`
                    ]),
                    columns: [
                        { title: "Employee" },
                        { title: "Subject" },
    ...
    const handleEdit = (event) => {
        const ContractId = $(event.currentTarget).data('id');
...
const handleSubmit = (e) => {
    ...
    axios.post('https://shawr1999.pythonanywhere.com//api/contracts-create/', formData)
        .then(response => {
            Swal.fire({
                title: "Good job!",
        ...
        setContractId(response.data.Leave_id);
      })
      .catch(error => {
  ...
  if (result.isConfirmed) {
    // 
    const ContractId = $(event.currentTarget).data('id');
        fetch(`https://shawr1999.pythonanywhere.com//api/contracts/${ContractId}/`, { method: 'DELETE' })
            .then(response => {
                if (response.ok) {
                    setContract(Contract.filter(Contract => Contract.id !== ContractId));
                    Swal.fire({
      title: "Deleted!",
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <input type="text" className='form-control' value={Employee_name} onChange={(e) => setEmployee_name(e.target.value)} required />
        </div>
          <div className="form-group">  
        <label>
         Contract Name:
