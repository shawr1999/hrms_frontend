...
export default function Appraisal_create() {
...
//   const [branchAddress, setBranchAddress] = useState('');
  ...
  const [AppraisalId, setAppraisalId] = useState('');
    ...
    useEffect(() => {
        fetch('https://shawr1999.pythonanywhere.com//api/appraisals/')
            ...
            .then(data => {
                ...
                const table = $(`#${tableName}`).DataTable({
                    data: data.map(Appraisal => [
                        ...
                        `<button class="edit-btn btn btn-primary" data-id="${Appraisal.id}">Edit</button>
                         <button class="delete-btn btn btn-danger" data-id="${Appraisal.id}">Delete</button>`
                    ]),
                    columns: [
                        { title: "Branch" },
                        { title: "Employee" },
    ...
    const handleEdit = (event) => {
        const AppraisalId = $(event.currentTarget).data('id');
        navigate(`/edit-Appraisal/${AppraisalId}`);
    };
...
const handleSubmit = (e) => {
    ...
    axios.post('https://shawr1999.pythonanywhere.com//api/appraisals-create/', formData)
        .then(response => {
            Swal.fire({
                title: "Good job!",
        ...
        setAppraisalId(response.data.Leave_id);
      })
      .catch(error => {
  ...
  if (result.isConfirmed) {
    // 
    const AppraisalId = $(event.currentTarget).data('id');
        fetch(`https://shawr1999.pythonanywhere.com//api/appraisals/${AppraisalId}/`, { method: 'DELETE' })
            .then(response => {
                if (response.ok) {
                    setAppraisal(Appraisal.filter(Appraisal => Appraisal.id !== AppraisalId));
                    Swal.fire({
      title: "Deleted!",
