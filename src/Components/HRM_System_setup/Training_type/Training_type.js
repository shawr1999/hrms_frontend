...
export default function Training_type_create() {
...
//   const [branchAddress, setBranchAddress] = useState('');
  ...
  const [Training_typeId, setTraining_typeId] = useState('');

  

    useEffect(() => {
        fetch('https://shawr1999.pythonanywhere.com//api/training_type_view/')
            .then(response => response.json())
            .then(data => {
                setTraining_typeId(data);
                const table = $(`#${tableName}`).DataTable({
                    data: data.map(Training_type => [
                        Training_type.Training_name,
                        // branch.branch_state,
                        // branch.branch_district,
                        // branch.branch_address,
                        `<button class="edit-btn btn btn-primary" data-id="${Training_type.id}">Edit</button>
                         <button class="delete-btn btn btn-danger" data-id="${Training_type.id}">Delete</button>`
                    ]),
                    columns: [
                        { title: "Training name" },
                        // { title: "State" },
    ...
    const handleEdit = (event) => {
        const Training_typeId = $(event.currentTarget).data('id');
        navigate(`/edit-Training_type/${Training_typeId}`);
    };
...
const handleSubmit = (e) => {
    ...
    axios.post('https://shawr1999.pythonanywhere.com//api/training_type_create/', formData)
        .then(response => {
            Swal.fire({
                title: "Good job!",
        ...
        setTraining_typeId(response.data.Leave_id);
      })
      .catch(error => {
  ...
  if (result.isConfirmed) {
    // 
    const Training_typeId = $(event.currentTarget).data('id');
        fetch(`https://shawr1999.pythonanywhere.com//api/training_type_view/${Training_typeId}/`, { method: 'DELETE' })
            .then(response => {
                if (response.ok) {
                    setTraining_type(Training_type.filter(Training_type => Training_type.id !== Training_typeId));
                    Swal.fire({
      title: "Deleted!",
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <input type="text" className='form-control' value={Training_name} onChange={(e) => setTraining_name(e.target.value)} required />
        </div>
          {/* <div className="form-group">  
        <label>
          Required Feild:
