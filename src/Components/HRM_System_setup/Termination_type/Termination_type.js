...
export default function Termination_type_create() {
...
//   const [branchAddress, setBranchAddress] = useState('');
  ...
  const [Termination_typeId, setTermination_typeId] = useState('');

  

    useEffect(() => {
        fetch('https://shawr1999.pythonanywhere.com//api/termination_type_view/')
            .then(response => response.json())
            .then(data => {
                setTermination_typeId(data);
                const table = $(`#${tableName}`).DataTable({
                    data: data.map(Termination_type => [
                        Termination_type.Termination,
                        // branch.branch_state,
                        // branch.branch_district,
                        // branch.branch_address,
                        `<button class="edit-btn btn btn-primary" data-id="${Termination_type.id}">Edit</button>
                         <button class="delete-btn btn btn-danger" data-id="${Termination_type.id}">Delete</button>`
                    ]),
                    columns: [
    ...
    const handleEdit = (event) => {
        const Termination_typeId = $(event.currentTarget).data('id');
        navigate(`/edit-Termination_type/${Termination_typeId}`);
    };
...
const handleSubmit = (e) => {
    ...
    axios.post('https://shawr1999.pythonanywhere.com//api/termination_type_create/', formData)
        .then(response => {
            Swal.fire({
                title: "Good job!",
        ...
        setTermination_typeId(response.data.Leave_id);
      })
      .catch(error => {
  ...
  if (result.isConfirmed) {
    // 
    const Termination_typeId = $(event.currentTarget).data('id');
        fetch(`https://shawr1999.pythonanywhere.com//api/termination_type_view/${Termination_typeId}/`, { method: 'DELETE' })
            .then(response => {
                if (response.ok) {
                    setTermination_type(Termination_type.filter(Termination_type => Termination_type.id !== Termination_typeId));
                    Swal.fire({
      title: "Deleted!",
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <input type="text" className='form-control' value={Termination} onChange={(e) => setTermination(e.target.value)} required />
        </div>
          {/* <div className="form-group">  
        <label>
          Required Feild:
