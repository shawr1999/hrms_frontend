...
export default function Goal_type_create() {
...
//   const [branchAddress, setBranchAddress] = useState('');
  ...
  const [Goal_typeId, setGoal_typeId] = useState('');

  

    useEffect(() => {
        fetch('https://shawr1999.pythonanywhere.com//api/goal_type_view/')
            .then(response => response.json())
            .then(data => {
                setGoal_typeId(data);
                const table = $(`#${tableName}`).DataTable({
                    data: data.map(Goal_type => [
                        Goal_type.Goal_name,
                        // branch.branch_state,
                        // branch.branch_district,
                        // branch.branch_address,
                        `<button class="edit-btn btn btn-primary" data-id="${Goal_type.id}">Edit</button>
                         <button class="delete-btn btn btn-danger" data-id="${Goal_type.id}">Delete</button>`
                    ]),
                    columns: [
                        { title: "Goal name" },
                        // { title: "State" },
    ...
    const handleEdit = (event) => {
        const Goal_typeId = $(event.currentTarget).data('id');
        navigate(`/edit-Goal_type/${Goal_typeId}`);
    };
...
const handleSubmit = (e) => {
    ...
    axios.post('https://shawr1999.pythonanywhere.com//api/goal_type_create/', formData)
        .then(response => {
            Swal.fire({
                title: "Good job!",
        ...
        setGoal_typeId(response.data.Leave_id);
      })
      .catch(error => {
  ...
  if (result.isConfirmed) {
    // 
    const Goal_typeId = $(event.currentTarget).data('id');
        fetch(`https://shawr1999.pythonanywhere.com//api/goal_type_view/${Goal_typeId}/`, { method: 'DELETE' })
            .then(response => {
                if (response.ok) {
                    setGoal_type(Goal_type.filter(Goal_type => Goal_type.id !== Goal_typeId));
                    Swal.fire({
      title: "Deleted!",
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <input type="text" className='form-control' value={Goal_name} onChange={(e) => setGoal_name(e.target.value)} required />
        </div>
          {/* <div className="form-group">  
        <label>
          Required Feild:
