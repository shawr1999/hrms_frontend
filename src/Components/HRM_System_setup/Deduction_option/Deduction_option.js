...
export default function Deduction_option_create() {
    const navigate = useNavigate();
    const [Deduction_option, setDeduction_option] = useState([]);
    const tableRef = useRef();
    const tableName = "table1";
    const [Deduction_option_name, setDeduction_option_name] = useState('');
//   const [branchState, setBranchState] = useState('');
//   const [branchDistrict, setBranchDistrict] = useState('');
//   const [branchAddress, setBranchAddress] = useState('');
  const [responseMessage, setResponseMessage] = useState('');
  const [Deduction_optionId, setDeduction_optionId] = useState('');

  

    useEffect(() => {
        fetch('https://shawr1999.pythonanywhere.com//api/deduction_option_view/')
            .then(response => response.json())
            .then(data => {
                setDeduction_optionId(data);
                const table = $(`#${tableName}`).DataTable({
                    data: data.map(Deduction_option => [
                        Deduction_option.Deduction_option_name,
                        // branch.branch_state,
                        // branch.branch_district,
                        // branch.branch_address,
                        `<button class="edit-btn btn btn-primary" data-id="${Deduction_option.id}">Edit</button>
                         <button class="delete-btn btn btn-danger" data-id="${Deduction_option.id}">Delete</button>`
                    ]),
                    columns: [
    ...
    const handleEdit = (event) => {
        const Deduction_optionId = $(event.currentTarget).data('id');
        navigate(`/edit-deduction_option/${Deduction_optionId}`);
    };

const handleSubmit = (e) => {
    e.preventDefault();
    const formData = {
        Deduction_option_name: Deduction_option_name
    };

    axios.post('https://shawr1999.pythonanywhere.com//api/deduction_option_create/', formData)
        .then(response => {
            Swal.fire({
                title: "Good job!",
        ...
        setDeduction_optionId(response.data.Leave_id);
      })
      .catch(error => {
  ...
  if (result.isConfirmed) {
    // 
    const Deduction_optionId = $(event.currentTarget).data('id');
        fetch(`https://shawr1999.pythonanywhere.com//api/deduction_option_view/${Deduction_optionId}/`, { method: 'DELETE' })
            .then(response => {
                if (response.ok) {
                    setDeduction_option(Deduction_option.filter(Deduction_option => Deduction_option.id !== Deduction_optionId));
                    Swal.fire({
      title: "Deleted!",
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <input type="text" className='form-control' value={Deduction_option_name} onChange={(e) => setDeduction_option_name(e.target.value)} required />
        </div>
          {/* <div className="form-group">  
        <label>
          Required Feild:
