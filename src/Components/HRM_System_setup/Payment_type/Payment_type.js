...
export default function Payment_type_create() {
    const navigate = useNavigate();
    const [Payment_type, setPayment_type] = useState([]);
    const tableRef = useRef();
    const tableName = "table1";
    const [Payment_type_name, setPayment_type_name] = useState('');
//   const [branchState, setBranchState] = useState('');
//   const [branchDistrict, setBranchDistrict] = useState('');
//   const [branchAddress, setBranchAddress] = useState('');
  const [responseMessage, setResponseMessage] = useState('');
  const [Payment_typeId, setPayment_typeId] = useState('');

  

    useEffect(() => {
        fetch('https://shawr1999.pythonanywhere.com//api/payment_type_view/')
            .then(response => response.json())
            .then(data => {
                setPayment_typeId(data);
                const table = $(`#${tableName}`).DataTable({
                    data: data.map(Payment_type => [
                        Payment_type.Payment_type_name,
                        // branch.branch_state,
                        // branch.branch_district,
                        // branch.branch_address,
                        `<button class="edit-btn btn btn-primary" data-id="${Payment_type.id}">Edit</button>
                         <button class="delete-btn btn btn-danger" data-id="${Payment_type.id}">Delete</button>`
                    ]),
                    columns: [
                        { title: "Training name" },
                        // { title: "State" },
    ...
    const handleEdit = (event) => {
        const Payment_typeId = $(event.currentTarget).data('id');
        navigate(`/edit-Payment_type/${Payment_typeId}`);
    };
...
const handleSubmit = (e) => {
    ...
    axios.post('https://shawr1999.pythonanywhere.com//api/payment_type_create/', formData)
        .then(response => {
            Swal.fire({
                title: "Good job!",
        ...
        setPayment_typeId(response.data.Leave_id);
      })
      .catch(error => {
  ...
  if (result.isConfirmed) {
    // 
    const Payment_typeId = $(event.currentTarget).data('id');
        fetch(`https://shawr1999.pythonanywhere.com//api/payment_type_view/${Payment_typeId}/`, { method: 'DELETE' })
            .then(response => {
                if (response.ok) {
                    setPayment_type(Payment_type.filter(Payment_type => Payment_type.id !== Payment_typeId));
                    Swal.fire({
      title: "Deleted!",
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <input type="text" className='form-control' value={Payment_type_name} onChange={(e) => setPayment_type_name(e.target.value)} required />
        </div>
          {/* <div className="form-group">  
        <label>
          Required Feild:
