...
export default function Income_type_create() {
    const navigate = useNavigate();
    const [Income_type, setIncome_type] = useState([]);
    const tableRef = useRef();
    const tableName = "table1";
    const [Income_type_name, setIncome_type_name] = useState('');
//   const [branchState, setBranchState] = useState('');
//   const [branchDistrict, setBranchDistrict] = useState('');
//   const [branchAddress, setBranchAddress] = useState('');
  const [responseMessage, setResponseMessage] = useState('');
  const [Income_typeId, setIncome_typeId] = useState('');

  

    useEffect(() => {
        fetch('https://shawr1999.pythonanywhere.com//api/income_type_view/')
            .then(response => response.json())
            .then(data => {
                setIncome_typeId(data);
                const table = $(`#${tableName}`).DataTable({
                    data: data.map(Income_type => [
                        Income_type.Income_type_name,
                        // branch.branch_state,
                        // branch.branch_district,
                        // branch.branch_address,
                        `<button class="edit-btn btn btn-primary" data-id="${Income_type.id}">Edit</button>
                         <button class="delete-btn btn btn-danger" data-id="${Income_type.id}">Delete</button>`
                    ]),
                    columns: [
                        { title: "Training name" },
                        // { title: "State" },
    ...
    const handleEdit = (event) => {
        const Income_typeId = $(event.currentTarget).data('id');
        navigate(`/edit-Income_type/${Income_typeId}`);
    };
...
const handleSubmit = (e) => {
    ...
    axios.post('https://shawr1999.pythonanywhere.com//api/income_type_create/', formData)
        .then(response => {
            Swal.fire({
                title: "Good job!",
        ...
        setIncome_typeId(response.data.Leave_id);
      })
      .catch(error => {
  ...
  if (result.isConfirmed) {
    // 
    const Income_typeId = $(event.currentTarget).data('id');
        fetch(`https://shawr1999.pythonanywhere.com//api/income_type_view/${Income_typeId}/`, { method: 'DELETE' })
            .then(response => {
                if (response.ok) {
                    setIncome_type(Income_type.filter(Income_type => Income_type.id !== Income_typeId));
                    Swal.fire({
      title: "Deleted!",
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <input type="text" className='form-control' value={Income_type_name} onChange={(e) => setIncome_type_name(e.target.value)} required />
        </div>
          {/* <div className="form-group">  
        <label>
          Required Feild:
