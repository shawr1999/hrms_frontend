...
export default function Contract_type_create() {
    const navigate = useNavigate();
    const [Contract_type, setContract_type] = useState([]);
    const tableRef = useRef();
    const tableName = "table1";
    const [Contract_type_name, setContract_type_name] = useState('');
//   const [branchState, setBranchState] = useState('');
//   const [branchDistrict, setBranchDistrict] = useState('');
//   const [branchAddress, setBranchAddress] = useState('');
  const [responseMessage, setResponseMessage] = useState('');
  const [Contract_typeId, setContract_typeId] = useState('');

  

    useEffect(() => {
        fetch('https://shawr1999.pythonanywhere.com//api/contract_type_view/')
            .then(response => response.json())
            .then(data => {
                setContract_typeId(data);
                const table = $(`#${tableName}`).DataTable({
                    data: data.map(Contract_type => [
                        Contract_type.Contract_type_name,
                        // branch.branch_state,
                        // branch.branch_district,
                        // branch.branch_address,
                        `<button class="edit-btn btn btn-primary" data-id="${Contract_type.id}">Edit</button>
                         <button class="delete-btn btn btn-danger" data-id="${Contract_type.id}">Delete</button>`
                    ]),
                    columns: [
                        { title: "Training name" },
                        // { title: "State" },
    ...
    const handleEdit = (event) => {
        const Contract_typeId = $(event.currentTarget).data('id');
        navigate(`/edit-Contract_type/${Contract_typeId}`);
    };
...
const handleSubmit = (e) => {
    ...
    axios.post('https://shawr1999.pythonanywhere.com//api/contract_type_create/', formData)
        .then(response => {
            Swal.fire({
                title: "Good job!",
        ...
        setContract_typeId(response.data.Leave_id);
      })
      .catch(error => {
  ...
  if (result.isConfirmed) {
    // 
    const Contract_typeId = $(event.currentTarget).data('id');
        fetch(`https://shawr1999.pythonanywhere.com//api/contract_type_view/${Contract_typeId}/`, { method: 'DELETE' })
            .then(response => {
                if (response.ok) {
                    setContract_type(Contract_type.filter(Contract_type => Contract_type.id !== Contract_typeId));
                    Swal.fire({
      title: "Deleted!",
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <input type="text" className='form-control' value={Contract_type_name} onChange={(e) => setContract_type_name(e.target.value)} required />
        </div>
          {/* <div className="form-group">  
        <label>
          Required Feild:
