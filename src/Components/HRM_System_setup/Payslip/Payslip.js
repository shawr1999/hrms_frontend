...
export default function Payslip_type_create() {
    const navigate = useNavigate();
    const [Payslip_type, setPayslip_type] = useState([]);
    const tableRef = useRef();
    const tableName = "table1";
    const [Payslip_name, setPayslip_name] = useState('');
//   const [branchState, setBranchState] = useState('');
//   const [branchDistrict, setBranchDistrict] = useState('');
//   const [branchAddress, setBranchAddress] = useState('');
  const [responseMessage, setResponseMessage] = useState('');
  const [Payslip_typeId, setPayslip_typeId] = useState('');

  

    useEffect(() => {
        fetch('https://shawr1999.pythonanywhere.com//api/payslip_type_view/')
            .then(response => response.json())
            .then(data => {
                setPayslip_typeId(data);
                const table = $(`#${tableName}`).DataTable({
                    data: data.map(Payslip_type => [
                        Payslip_type.payslip_name,
                        // branch.branch_state,
                        // branch.branch_district,
                        // branch.branch_address,
                        `<button class="edit-btn btn btn-primary" data-id="${Payslip_type.id}">Edit</button>
                         <button class="delete-btn btn btn-danger" data-id="${Payslip_type.id}">Delete</button>`
                    ]),
                    columns: [
                        { title: "Payslip Name" },
                        // { title: "State" },
    ...
    const handleEdit = (event) => {
        const Payslip_typeId = $(event.currentTarget).data('id');
        navigate(`/edit-payslip-type/${Payslip_typeId}`);
    };
...
const handleSubmit = (e) => {
    ...
    axios.post('https://shawr1999.pythonanywhere.com//api/payslip_type_create/', formData)
        .then(response => {
            Swal.fire({
                title: "Good job!",
        ...
        setPayslip_typeId(response.data.Leave_id);
      })
      .catch(error => {
  ...
  if (result.isConfirmed) {
    // 
    const Payslip_typeId = $(event.currentTarget).data('id');
        fetch(`https://shawr1999.pythonanywhere.com//api/payslip_type_view/${Payslip_typeId}/`, { method: 'DELETE' })
            .then(response => {
                if (response.ok) {
                    setPayslip_type(Payslip_type.filter(Payslip_type => Payslip_type.id !== Payslip_typeId));
                    Swal.fire({
      title: "Deleted!",
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <input type="text" className='form-control' value={Payslip_name} onChange={(e) => setPayslip_name(e.target.value)} required />
        </div>
          {/* <div className="form-group">  
        <label>
          Required Feild:
