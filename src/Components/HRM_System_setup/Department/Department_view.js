            ...
            .then(data => {
                ...
                const table = $(`#${tableName}`).DataTable({
                    data: data.map(department => [
                        ...
                        `<button class="edit-btn btn btn-primary" data-id="${department.id}">Edit</button>
                         <button class="delete-btn btn btn-danger" data-id="${department.id}">Delete</button>`
                    ]),
                    columns: [
...
const handleSubmit = (e) => {
    ...
    axios.post('https://shawr1999.pythonanywhere.com//api/department_create/', formData)
        .then(response => {
            Swal.fire({
                title: "Good job!",
                text: response.data.message + "with Department Id:-" + response.data.department_id,
                icon: "success",
  ...
  if (result.isConfirmed) {
    // 
    const DepartmentId = $(event.currentTarget).data('id');
        fetch(`https://shawr1999.pythonanywhere.com//api/department_view/${DepartmentId}/`, { method: 'DELETE' })
            .then(response => {
                if (response.ok) {
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <input type="text" className='form-control' value={DepartmentCategory} onChange={(e) => setDepartmentCategory(e.target.value)} required />
        </div>
          <div className="form-group">  
        <label>
          Department Name:
        </label>
        <input type="text" className='form-control' value={DepartmentName} onChange={(e) => setDepartmentName(e.target.value)} required />
        </div>
    {/* <div className="form-group">
        <label>
          Branch Country:
