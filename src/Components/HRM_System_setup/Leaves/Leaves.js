...
export default function Leaves() {
  ...
  const [Days_Per_Year, setDays_Per_Year] = useState('');
...
//   const [branchAddress, setBranchAddress] = useState('');
  ...
  const [LeaveId, setLeaveId] = useState('');
        ...
        fetch('https://shawr1999.pythonanywhere.com//api/leave_view/')
            ...
            .then(data => {
                ...
                const table = $(`#${tableName}`).DataTable({
                    data: data.map(leaves => [
                        leaves.leave_name,
                        leaves.days_per_year,
                        // branch.branch_state,
                        // branch.branch_district,
                        // branch.branch_address,
                        `<button class="edit-btn btn btn-primary" data-id="${leaves.id}">Edit</button>
                         <button class="delete-btn btn btn-danger" data-id="${leaves.id}">Delete</button>`
                    ]),
                    columns: [
                        { title: "Leave Name" },
                        { title: "Days Per Year" },
    ...
    const handleEdit = (event) => {
        const LeaveId = $(event.currentTarget).data('id');
...
const handleSubmit = (e) => {
    ...
    axios.post('https://shawr1999.pythonanywhere.com//api/leave_create/', formData)
        .then(response => {
            Swal.fire({
                title: "Good job!",
        ...
        setLeaveId(response.data.Leave_id);
      })
      .catch(error => {
  ...
  if (result.isConfirmed) {
    // 
    const LeaveId = $(event.currentTarget).data('id');
        fetch(`https://shawr1999.pythonanywhere.com//api/leave_view/${LeaveId}/`, { method: 'DELETE' })
            .then(response => {
                if (response.ok) {
                    setLeaves(Leaves.filter(Leaves => Leaves.id !== LeaveId));
                    Swal.fire({
      title: "Deleted!",
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <input type="text" className='form-control' value={LeaveName} onChange={(e) => setLeaveName(e.target.value)} required />
        </div>
          <div className="form-group">  
        <label>
          Days Per Year:
        </label>
        <input type="text" className='form-control' value={Days_Per_Year} onChange={(e) => setDays_Per_Year(e.target.value)} required />
        </div>
    {/* <div className="form-group">
        <label>
          Branch Country:
