...
export default function Document_type_create() {
    const navigate = useNavigate();
    const [Document_type, setDocument_type] = useState([]);
    const tableRef = useRef();
    const tableName = "table1";
    const [document_name, setdocument_name] = useState('');
  const [required_feild, setrequired_feild] = useState('');
//   const [branchState, setBranchState] = useState('');
//   const [branchDistrict, setBranchDistrict] = useState('');
//   const [branchAddress, setBranchAddress] = useState('');
  const [responseMessage, setResponseMessage] = useState('');
  const [Document_typeId, setDocument_typeId] = useState('');

  

    useEffect(() => {
        fetch('https://shawr1999.pythonanywhere.com//api/document_type_view/')
            .then(response => response.json())
            .then(data => {
                setDocument_typeId(data);
                const table = $(`#${tableName}`).DataTable({
                    data: data.map(Document_type => [
                        Document_type.document_name,
                        Document_type.required_feild,
                        // branch.branch_state,
                        // branch.branch_district,
                        // branch.branch_address,
                        `<button class="edit-btn btn btn-primary" data-id="${Document_type.id}">Edit</button>
                         <button class="delete-btn btn btn-danger" data-id="${Document_type.id}">Delete</button>`
                    ]),
                    columns: [
                        { title: "Document Name" },
                        { title: "Required" },
    ...
    const handleEdit = (event) => {
        const Document_typeId = $(event.currentTarget).data('id');
        navigate(`/edit-document-type/${Document_typeId}`);
    };
...
const handleSubmit = (e) => {
    ...
    const formData = {
      ...
      required_feild: required_feild
    };

    axios.post('https://shawr1999.pythonanywhere.com//api/document_type_create/', formData)
        .then(response => {
            Swal.fire({
                title: "Good job!",
        ...
        setDocument_typeId(response.data.Leave_id);
      })
      .catch(error => {
  ...
  if (result.isConfirmed) {
    // 
    const Document_typeId = $(event.currentTarget).data('id');
        fetch(`https://shawr1999.pythonanywhere.com//api/document_type_view/${Document_typeId}/`, { method: 'DELETE' })
            .then(response => {
                if (response.ok) {
                    setDocument_type(Document_type.filter(Document_type => Document_type.id !== Document_typeId));
                    Swal.fire({
      title: "Deleted!",
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <input type="text" className='form-control' value={document_name} onChange={(e) => setdocument_name(e.target.value)} required />
        </div>
          <div className="form-group">  
        <label>
          Required Feild:
