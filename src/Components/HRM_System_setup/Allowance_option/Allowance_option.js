...
export default function Allowance_option_create() {
    const navigate = useNavigate();
    const [Allowance_option, setAllowance_option] = useState([]);
    const tableRef = useRef();
    const tableName = "table1";
    const [Allowance_option_name, setAllowance_option_name] = useState('');
//   const [branchState, setBranchState] = useState('');
//   const [branchDistrict, setBranchDistrict] = useState('');
//   const [branchAddress, setBranchAddress] = useState('');
  const [responseMessage, setResponseMessage] = useState('');
  const [Allowance_optionId, setAllowance_optionId] = useState('');

  

    useEffect(() => {
        fetch('https://shawr1999.pythonanywhere.com//api/allowance_option_view/')
            .then(response => response.json())
            .then(data => {
                setAllowance_optionId(data);
                const table = $(`#${tableName}`).DataTable({
                    data: data.map(Allowance_option => [
                        Allowance_option.allowance_option_name,
                        // branch.branch_state,
                        // branch.branch_district,
                        // branch.branch_address,
                        `<button class="edit-btn btn btn-primary" data-id="${Allowance_option.id}">Edit</button>
                         <button class="delete-btn btn btn-danger" data-id="${Allowance_option.id}">Delete</button>`
                    ]),
                    columns: [
    ...
    const handleEdit = (event) => {
        const Allowance_optionId = $(event.currentTarget).data('id');
        navigate(`/edit-allowance_option/${Allowance_optionId}`);
    };

const handleSubmit = (e) => {
    e.preventDefault();
    const formData = {
        allowance_option_name: Allowance_option_name
    };

    axios.post('https://shawr1999.pythonanywhere.com//api/allowance_option_create/', formData)
        .then(response => {
            Swal.fire({
                title: "Good job!",
        ...
        setAllowance_optionId(response.data.Leave_id);
      })
      .catch(error => {
  ...
  if (result.isConfirmed) {
    // 
    const Allowance_optionId = $(event.currentTarget).data('id');
        fetch(`https://shawr1999.pythonanywhere.com//api/allowance_option_view/${Allowance_optionId}/`, { method: 'DELETE' })
            .then(response => {
                if (response.ok) {
                    setAllowance_option(Allowance_option.filter(Allowance_option => Allowance_option.id !== Allowance_optionId));
                    Swal.fire({
      title: "Deleted!",
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <input type="text" className='form-control' value={Allowance_option_name} onChange={(e) => setAllowance_option_name(e.target.value)} required />
        </div>
          {/* <div className="form-group">  
        <label>
          Required Feild:
